...
static void OpenMessageProc(XIMS ims, IMProtocol *call_data, unsigned char *p)
{
    ...
    for (i = 0;  i < i18n_core->address.im_attr_num;  i++)
    {
        ...
        /* printf("im %d %s\n",str_size,i18n_core->address.xim_attr[i].name); */
    }
    /*endfor*/
    for (i = 0;  i < i18n_core->address.ic_attr_num;  i++)
    {
        ...
        /* printf("ic %s\n",i18n_core->address.xic_attr[i].name); */
    }
    /*endfor*/
    _Xi18nSendMessage (ims,
                       connect_id,
                                          ...
                                          int *length)
{
    ...
    *length = 0;
    ...
    for (i = 0;  i < *number;  i++)
    {
		...
		// dgod: it may not init at GetIMValueFromName
		value_length=0;
                           ...
                           Bool *delete)
{
	...
	DebugLog("-- XIM_RESET_IC\n");
		...
		/* dgod:
		 * for some client don't XIM_SYNC_REPLY sometimes at new IC, and just reset IC
		 * so we should clear the sync bit at client here
		 */
		client->sync=0;
        break;
